        ...
        <table class="table mt-3">
            <tbody>
                <tr>
                    ...
                    <td class="text-right">
                        <a class="btn btn-success" href="{{ route('vendor.edit',$vendor->vendor_id) }}">EditSupplier</a>
                    </td>
                </tr>
                <tr>
                    <th scope="col">PIC</th>
                    <td colspan="2">{{ isset($vendor->detail->pic_name) ? $vendor->detail->pic_name : ''}}</td>
                </tr>
                <tr>
                    <th scope="row">Supplier Address</th>
                    <td colspan="2">{{$vendor->vendor_address}}</td>
                </tr>
                <tr>
                    <th scope="row">Office Phone Number</th>
                    <td colspan="2">{{ isset($vendor->detail->no_telp_office) ? $vendor->detail->no_telp_office : '' }}</td>
                </tr>
                <tr>
                    <th scope="row">Mobile Phone</th>
                    <td colspan="2">{{ isset($vendor->detail->no_telp_hp ) ? $vendor->detail->no_telp_hp  : '' }}</td>
                </tr>

                <tr>
                    <th scope="row">Email/surel</th>
                    <td colspan="2">{{ isset($vendor->detail->email) ? $vendor->detail->email : '' }}</td>

                </tr>
                <tr>
                    <th scope="row">Website/situs web</th>
                    <td colspan="2">{{ isset($vendor->detail->link_website) ? $vendor->detail->link_website : '' }}</td>
                </tr>

                <tr>
                    <th scope="row">Production Capacity</th>
                    <td colspan="2">{{ isset($vendor->detail->kapasitas_produksi) ? $vendor->detail->kapasitas_produksi : '' }}</td>
                </tr>
                <tr>
                    <th scope="row">Document Status</th>
                    <td><b>{!! $vendor->document_status_name !!}</b></td>
                    <td class="text-right">
                        <form class="text-right" method="POST" action="{{ route('document.status', request()->route()->vendor) }}">
                            @csrf
                            ...
                            <button type="submit" name="status" value="3" class="btn btn-danger">
                                PKS Reject to Sent The Document List
                            </button>
                            @endif
                        </form>
                    </td>
